...
//在textarea选中位置插入给定的字符
//参数:tid:textarea的id;str:要插入的字符串
function insertString(tid,str)
{
	...
	if (document.selection) {
        ...
        //MOZILLA/NETSCAPE support
            ...
            textarea.value = chaineSql.substring(0, startPos) + str + chaineSql.substring(endPos, chaineSql.length);
        } else {
            textarea.value += str;
        }
}
...
//列表组建更多操作
function moreAction(e,id){
	if(!is_string(moreActionList))
		return;
	var click_event = e || event;
	var listX=Event.pointerX(click_event)-150;
	var listY=Event.pointerY(click_event)+20;
	var moreActionDiv=document.getElementById("moreAction");
	var actions=moreActionList.split(",");
	var html="<ul>";
	for(i=0;i<actions.length;i++){
		action=actions[i].split(':');
		html+="<li><label><a href='javascript:"+action[0]+"("+id+")'>"+action[1]+"</a></label></li>";
	}
	html+="<li><a href='#'onclick='document.getElementById(\"moreAction\").style.display=\"none\";return false;'>【关闭更多操作】</a></li>";
	html+='</ul>';
	moreActionDiv.innerHTML=html;
	moreActionDiv.style.position="absolute";
	moreActionDiv.style.left=listX+"px";
	moreActionDiv.style.top=listY+"px";
